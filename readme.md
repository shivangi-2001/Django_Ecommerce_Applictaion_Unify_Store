...
| ![photo-12](photos/p-12.png) | ![photo-13](photos/p-13.png) |
|-------------------------------|-------------------------------|

#### 1. OTP-based Account Activation

   * Users must confirm their email with an OTP sent via `django-otp`. 
   * Ensures only verified emails can log in.

#### 2. Automatic Lockout after Failed OTP Attempts 
...
## Password Security
The application implements a secure password reset mechanism with tokenized links and cooldown restrictions.

#### 1. Password Reset Request

* User enters their registered and active email on the login page.
* If valid, a password reset link is generated with a time-limited token.
* Link is emailed to the user and is valid for 30 minutes.
* To prevent spamming, users can only request a reset link once every 3 minutes (cooldown).

#### 2. Token & Expiration

* Each reset link is associated with a unique token generated by reset_password_token.

* The link is only valid for 30 minutes from the request time.

* If expired, the user must request a new reset link.

#### 3. Reset Form

* When the user clicks the reset link, they are directed to the Reset Password form.

* They must enter a new password that complies with Django’s password validation rules.

* On success:
   * Password is updated.
   * Token and reset timer are cleared.
   * User is redirected to the login page with a success message.

#### 4. Security Controls

- ✅ Cooldown: New reset links can only be requested every 3 minutes.

- ✅ Expiration: Reset links automatically expire after 30 minutes.

- ✅ Validation: Inactive or non-existent accounts don’t reveal information; instead, a generic success message is shown.
